...
Type=CUSTOM_JAR,Name="mysqltos3",Jar=$CODEDIR/script-runner.jar,Args=["$CODEDIR/mysqltos3_sample.sh"] \
Type=Hive,Name="HiveStep",Args=[-f,$CODEDIR/hivetables_sample.q,-d,DISDIR=$DISDIR]  \
Type=CUSTOM_JAR,Name="s3tomysql",Jar=$CODEDIR/script-runner.jar,Args=["$CODEDIR/s3tomysql_sample.sh"]


#Type=CUSTOM_JAR,Name="init_env",Jar=$CODEDIR/script-runner.jar,Args=["$CODEDIR/int_env.sh"] \
