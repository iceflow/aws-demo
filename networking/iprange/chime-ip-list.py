...
def get_chime_ip_ranges():
    ''' 
        参考 chime.region.list chime 规则
        IPv4 only
    ''' 
    data = None
    ...
    filters = [
        {
            'region_list': ['ap-northeast-1', 'ap-southeast-1', 'us-east-1', 'us-east-2', 'us-west-1', 'us-west-2'],
            'service_list': ['EC2'],
            'ip_type': 'ipv4'
        },
        {
            'region_list': ['GLOBAL'],
            'service_list': ['ROUTE53'],
            'ip_type': 'ipv4'
        }
    ]
    ...
    for item in data[prefix_dict[ip_type]['prefixes']]:
        for policy in filters:
            if item['region'] in policy['region_list'] and item['service'] in policy['service_list']:
